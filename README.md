...
- Prompt-to-trait translation now runs through `POST /api/traits`, which calls OpenAI's Responses API with Structured Outputs. Set `OPENAI_API_KEY` before running the app. Override the default model with `OPENAI_TRAIT_MODEL` if needed.
